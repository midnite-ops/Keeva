import { Box, HeartIcon, Package, ShoppingBag } from "lucide-react";
        ...
        <div key={category} className="flex flex-col gap-10 mt-10 overflow-y-scroll">
          {items.map((item) => {
            if (item.type === "trending") {
              return (
                <div key={item.id} className="flex items-center relative gap-4 cursor-pointer md:w-3/4 w-full" onClick={() => markAsRead(item.id)}>
                  ...
                  {!item.read && <div className="bg-blue-800 size-2 rounded-full absolute right-0"></div>}
                </div>
              );
            }
            ...
            if (item.type === "restock") {
              return (
                <div key={item.id} className="flex relative gap-4  w-full md:w-3/4" onClick={() => markAsRead(item.id)}>
