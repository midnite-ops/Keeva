...
export default function AddPost() {
  ...
  const canAdvance = () => {
    ...
    if (step === 2)
      return (
        form.name.trim() !== "" && form.category !== "" && form.price !== ""
      );
    ...
    return (
      <div className="min-h-screen  bg-white flex items-center justify-center px-6">
        <div className="text-center w-full max-w-sm">
          ...
          >
            <strong className="text-black">{form.name}</strong> is now live in
            your store.
          </p>
          <button
            onClick={handleReset}
      ...
      <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm border-b border-neutral-100">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          ...
          <button
            ...
            disabled={step === 1}
            className="flex items-center gap-1.5 text-sm font-medium text-neutral-500 hover:text-black disabled:opacity-0 disabled:pointer-events-none transition-colors shrink-0"
            style={{ fontFamily: "Inter, system-ui, sans-serif" }}
          >
          ...
          {step < 4 ? (
            <button
              ...
              disabled={!canAdvance()}
              className="flex items-center gap-1.5 px-4 sm:px-5 py-2 bg-black text-white text-sm font-medium rounded-full hover:bg-neutral-800 disabled:opacity-30 disabled:cursor-not-allowed transition-all shrink-0"
              style={{ fontFamily: "Inter, system-ui, sans-serif" }}
            >
              Continue
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 sm:px-5 py-2 bg-black text-white text-sm font-medium rounded-full hover:bg-neutral-800 transition-colors shrink-0"
              style={{ fontFamily: "Inter, system-ui, sans-serif" }}
            >
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="mb-8 sm:mb-10">
          <p
            ...
            style={{ fontFamily: "Inter, system-ui, sans-serif" }}
          >
            {getCurrentUser()?.role === "brand" ? " Add Product" : "Add Outfit"}
          </p>
        ...
        <div className="min-h-[320px]">
          ...
          {step === 3 && (
            <InventoryStep
              ...
              onChange={(sizeStock) =>
                setForm((prev) => ({ ...prev, sizeStock }))
              }
            />
          )}
          {step === 4 && <ReviewStep data={form} coverPreview={coverPreview} />}
        </div>
