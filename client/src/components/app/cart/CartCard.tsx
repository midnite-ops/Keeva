...
import OutfitDetails from "./OutiftDetails";
          ...
          <div className="flex items-end justify-between mt-3">
            ...
            <div className="flex flex-col items-end gap-2">
              <span className="text-base font-semibold text-gray-900">
                ${item.type === "product" ? (item.price * item.stock.reduce((sum, item) => sum + item.quantity, 0)) : item.totalPrice}
              </span>
          ...
          {item.type === "product" && (
            <div className="flex items-center gap-2 mt-3">
              <div className="flex items-center rounded-lg overflow-hidden" style={{ border: "1px solid #ece9e4" }}>
                <button className="w-7 h-7 flex items-center justify-center text-gray-500 hover:bg-gray-50 transition-colors">
                  <Minus size={12} onClick={() => onQty(-1)} />
                </button>
                <span className="w-8 text-center text-sm font-semibold text-gray-900">{item.stock.reduce((sum, item) => sum + item.quantity, 0)}</span>
                <button className="w-7 h-7 flex items-center justify-center text-gray-500 hover:bg-gray-50 transition-colors">
                  <Plus size={12} onClick={() => onQty(1)} />
                </button>
              </div>
            </div>
          )}
