    ...
    >
            ...
            <div>
              {item.type === "outfits" && (
                <span
                  ...
                  style={{ background: "#f4f3fd", color: "#7c72c8" }}
                >
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {item.type === "product"
                  ...
                  : `by ${findUser(item.creatorId)?.username}`}
              </p>
            </div>
            <button
              onClick={onRemove}
            ...
            <div className="flex flex-col gap-1">
              ...
              {item.type === "outfits" && (
                <span className="text-xs text-gray-400">
                  {item.taggedProducts.length} pieces included
                </span>
              )}
            </div>
            <div className="flex flex-col items-end gap-2">
              <span className="text-base font-semibold text-gray-900">
                ...
                {item.type === "product"
                  ? (
                      item.price *
                      item.stock.reduce((sum, item) => sum + item.quantity, 0)
                    ).toLocaleString()
      ...
      <button
        ...
        style={{
          ...
          color: expanded ? "#111" : "#aaa",
          background: expanded ? "#fafaf9" : "transparent",
        }}
      >
        <span>
          {expanded
      ...
      {expanded && (
        <div
          className="px-4 pb-4"
          style={{ borderTop: "1px solid #f0ede8", background: "#fafaf9" }}
        >
          {item.type === "product" ? (
            <ProductDetails item={item} onSize={onSize} />
          ) : (
            <OutfitDetails item={item} />
          )}
        </div>
      )}
    </div>
