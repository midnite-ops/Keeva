...
export const ProfileCard = ({
  ...
  return (
    ...
    >
      {/* Look grid preview */}
      <div
        className={`relative grid grid-cols-3 gap-0.5  h-40 bg-secondary overflow-hidden [mask-image:linear-gradient(to_top,transparent_0%,black_30%)]
      ...
      <div className="px-5 pb-5 flex flex-col gap-4 flex-1">
        ...
        <div className="grid grid-cols-3 gap-2 border-t border-subtitleText pt-4">
          {[
            ...
            {
              label: "Looks",
              value: role === "creator" ? outfits?.length : products?.length,
            },
        ...
        <button
          ...
          className={`flex items-center justify-center gap-2 text-[10px] tracking-[0.18em] uppercase rounded-full py-2.5 transition-all duration-200 ${"border border-border text-muted-foreground bg-bgBlack hover:border-foreground/30 text-background"}`}
        >
